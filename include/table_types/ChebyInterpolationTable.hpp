 ...
 *   ChebyInterpolationTable<double,double,4> look(&function,LookupTableParameters<double>{0,10,0.0001});
...
class ChebyInterpolationTable final : public MetaTable<TIN,TOUT,N+1,GT>
{
...
public:
  ...
  ChebyInterpolationTable(FunctionContainer<TIN,TOUT> *func_container, LookupTableParameters<TIN> par,
      ...
      std::move(MetaTable<TIN,TOUT,N+1,GT>(jsonStats, classname, func_container)))
  {
    ...
    // throw a descriptive exception. This could theoretically be a compile time error; however,
    // that will only stop us from registering this table (not useful!)
    if(jsonStats.empty())
      throw std::invalid_argument("Chebyshev tables need Armadillo to be generated");
#else
    if(!jsonStats.empty())
...
const std::string ChebyInterpolationTable<TIN,TOUT,N,GT>::classname = grid_type_to_string<GT>() + "ChebyInterpolationTable<" + std::to_string(N) + ">";

// define friendlier names
template <typename TIN, typename TOUT, unsigned int N>
using UniformChebyInterpolationTable = ChebyInterpolationTable<TIN,TOUT,N,GridTypes::UNIFORM>;
template <typename TIN, typename TOUT, unsigned int N>
using NonUniformChebyInterpolationTable = ChebyInterpolationTable<TIN,TOUT,N,GridTypes::NONUNIFORM>;
template <typename TIN, typename TOUT, unsigned int N>
using NonUniformPseudoChebyInterpolationTable = ChebyInterpolationTable<TIN,TOUT,N,GridTypes::NONUNIFORM_PSEUDO>;
