...
UniformLookupTable::UniformLookupTable(EvaluationFunctor<boost_fvar,boost_fvar> *func, UniformLookupTableParameters par) :
  mp_boost_func(func), EvaluationImplementation(new FuncWrapper(this),"uniform_lookup_table")
{
  //mp_func = new FuncWrapper(this);
...
EvaluationFunctor<boost_fvar,boost_fvar> *UniformLookupTable::fvar_function(){ return mp_boost_func; }

//EvaluationFunctor<boost_fvar,boost_fvar> *UniformLookupTable::fvar_to_double_functor(){ return NULL; }

/* ////////////////////////////////////////////////////////////////////////////
   Factory and registration management
//////////////////////////////////////////////////////////////////////////// */
std::unique_ptr<UniformLookupTable> UniformLookupTableFactory::Create(std::string name,
						       EvaluationFunctor<boost_fvar,boost_fvar> *f,
						       UniformLookupTableParameters par)
{
  ...
  if(it != get_registry().end())
    instance = it->second(f,par); // TODO this line causes an exception for hermite tables
...
std::function<UniformLookupTable*(EvaluationFunctor<boost_fvar,boost_fvar>*,UniformLookupTableParameters)> classFactoryFunction)
{
...
std::map<std::string, std::function<UniformLookupTable*(
			       EvaluationFunctor<boost_fvar,boost_fvar>*,UniformLookupTableParameters)>>& UniformLookupTableFactory::get_registry()
{
  // Get the singleton instance of the registry map
  static std::map<std::string, std::function<UniformLookupTable*(
			       EvaluationFunctor<boost_fvar,boost_fvar>*,UniformLookupTableParameters)>> registry;
  return registry;
}
